<template>
    <v-list>
        <div v-for="route in routes" :key="route.path">
            ...
            <v-list-group :group="route.path" v-if="hasRouteChildren(route)" :class="isLevelOneRoute(route) ? '' : 'ml-5'">
                <template #activator>
            ...
            <div v-else>
                <v-list-item :to="routeNameTo(route)" :class="isLevelOneRoute(route) ? '' : 'ml-5'" :color="activeColor">
                    <v-list-item-icon v-if="listItemConfig(route, 'icon')">
                        <v-icon>{{ `mdi-${listItemConfig(route, 'icon')}` }}</v-icon>
                    </v-list-item-icon>
                    <v-list-item-title v-if="listItemConfig(route, 'title')"> {{ listItemConfig(route, 'title') }}</v-list-item-title>
                </v-list-item>
            </div>
        </div>
    </v-list>
...
export default {
    ...
    methods: {
        ...
        listItemConfig(route, property) {
            return route.children ? route.children[0]['meta'][property] : route.meta[property]
        },
        /**
